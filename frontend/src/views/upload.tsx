...
const UploadView = (): JSX.Element => {
  ...
  useEffect(() => {
    setFileUploader(new FileUploader(auth.uploadToken!.toString(), auth.uploadToken!.archiveId, auth.uploadSecret!));
  }, []);
  ...
  function filesChanged(addedFiles: File[]) {
    setFiles((files) => {
      ...
      for (let i = newFiles.length; i < addedFiles.length; i++) {
        newFiles[i] = { file: addedFiles[i], internalFileId: i.toString(), uploadProgress: 0, encryptionProgress: 0 };
  ...
  return <main className="flex flex-col flex-1 place-content-center items-center">
    <article className="bg-white py-10 px-8 rounded-xl">
      ...
      <Dropper onChange={filesChanged} maximumFileCount={maximumFileCount} maximumFileSize={maximumFileSize} maximumSize={maximumSize} />
