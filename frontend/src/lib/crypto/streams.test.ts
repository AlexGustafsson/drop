...
import { EncryptionStream, FileStream, DecryptionStream } from "./streams";
import { hexToBuffer, bufferToHex, hexToCryptoKey, } from "./utils";
...
function bufferToStream(buffer: ArrayBuffer, chunkSize: number): ReadableStream<ArrayBuffer> {
...
describe("encryption stream", () => {
  it("encrypts a stream", async () => {
    const key = await hexToCryptoKey("4dff1f50db3a861fce01c7004afd613317248f0895f723b9c11d4855a08621d6");
...
describe ("encryption / decryption stream", () => {
  it("survives roundtrip", async () => {
    ...
    const buffer = hexToBuffer("01020304050607080910121314151617");
    const stream = bufferToStream(buffer, 8);
    const reader = stream.pipeThrough(new EncryptionStream(key)).pipeThrough(new DecryptionStream(key)).getReader();
