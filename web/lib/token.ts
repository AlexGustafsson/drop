...
export class UploadToken extends Token {
  ...
  static parse(token: string): UploadToken {
    ...
    if (claims['iss'] !== 'drop') throw Error('Not a valid token for drop')
...
export class AdminToken extends Token {
  constructor(raw: string, expiresAt: Date, id: string, issuer: string) {
    super(raw, expiresAt, id, issuer)
  }
  ...
  static parse(token: string): AdminToken {
    const claims = JSON.parse(atob(token.split('.')[1]))
    if (claims['iss'] !== 'drop') throw Error('Not a valid token for drop')
    if (claims['sub'] !== 'admin') throw Error('Not a valid token for an admin')
