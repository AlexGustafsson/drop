...
export function useSnackbars(): SnackbarsInterface {
  ...
  const remove = (snackbar: UpdatableSnackbarMessage) => {
    setSnackbars((currentSnackbars) =>
      currentSnackbars.filter((x) => x.id !== snackbar.id)
    )
    if (snackbar.removed) {
...
export const Snackbar = ({ snackbar }: SnackbarProps): JSX.Element => {
  ...
  return (
    <div
      className={`animate-move-in relative flex flex-col justify-center rounded-md w-96 text-sm p-3 mb-2 overflow-hidden pointer-events-auto ${height} ${background}`}
    >
      ...
      <a
        ...
        onClick={() => snackbars.remove(snackbar)}
      >
        OK
      </a>
    </div>
...
export const SnackbarContainer = ({
  children,
}: HTMLAttributes<Provider<SnackbarState>>): JSX.Element => {
  const [snackbars, setSnackbars] = useState<UpdatableSnackbarMessage[]>([])
  const snackbarElements = snackbars.map((snackbar) => (
    <Snackbar key={snackbar.id} snackbar={snackbar}></Snackbar>
  ))
  return (
